...
/**
  * @brief 打开串口
  **/
bool MyCom::openPort(QString name, BaudRateType baudrate, DataBitsType databits,
                     ParityType parity, StopBitsType stopbits, FlowType flow)
{
    PortSettings settings = {baudrate, databits, parity, stopbits, flow, 10};
    port = new QextSerialPort(name, settings, QextSerialPort::EventDriven);
...
/**
  * @brief 关闭串口
  **/
bool MyCom::closePort()
{
    qDebug() << "Port is closed now.";
    port->close();
}
...
void MyCom::onReadyRead()
{
    ...
    while (!out.atEnd())
    {
        quint8 outchar = 0;
        out>>outchar;
        data_receive_ << outchar;
        if (receive_bits_++ >= 17)
        {
            if (checkCrc32(&data_receive_[0], 18))
                emit sigDisplayReceive(data_receive_, 1);
            else
                emit sigDisplayReceive(data_receive_, 0);
